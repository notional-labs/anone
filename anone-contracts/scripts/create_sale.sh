...
CREATE_SALE="{\"create_sale\": {\"contract\":$MARKETPLACE_CONTRACT_ADDR, \"token_id\":\"$TOKEN_ID\", \"msg\": \"$BASE64_ENCODED_JSON\"}}"
echo $CREATE_SALE

RES=$(anoned tx wasm execute "$CW721_CONTRACT_ADDR" "$CREATE_SALE" --from "$ACCOUNT" -y --output json --chain-id "$CHAINID" --node "$NODE" --gas="auto" --gas=auto --fees 875000uan1 -y --output json)
echo $RES
