<template>
      ...
      <ul class="navbar-nav mr-auto">
        <li class="nav-item">
          <button
            class="btn"
            :class="{
              'btn-primary': currentState == MARKET,
              'btn-inverse': currentState !== MARKET,
            }"
            @click="changeDisplayState(0)"
            :disabled="!accounts"
          >
            Market
          </button>
        ...
        <li class="nav-item">
          <button
            class="btn"
            :class="{
              'btn-primary': currentState == MINT,
              'btn-inverse': currentState !== MINT,
            }"
            @click="changeDisplayState(1)"
            :disabled="!accounts"
          >
            Mint
          </button>
        ...
        <li class="nav-item">
          <button
            class="btn"
            :class="{
              'btn-primary': currentState == VIEW_OWNER && !selectedOwner,
              'btn-inverse': currentState !== VIEW_OWNER,
            }"
            @click="changeDisplayState(3)"
            :disabled="!accounts"
          >
            My NFTs
          </button>
  ...
  <img alt="logo" src="./assets/anone-logo.png" />
  ...
  <div class="content" v-if="!accounts">
    ...
    <div class="button-controls">
      <!-- Connect -->
      <button id="connect" class="btn btn-main" @click="connectWallet()">
        Connect Wallet
      </button>
    </div>
  </div>
  ...
  <div class="content" v-else>
    ...
    <div class="market" v-if="currentState == MARKET">
      ...
      <div class="market-items" v-if="nfts.market">
        <div v-if="!nfts.market.tokens.length">
          <p>
            There are no NFTs in this collection, try
            <a class="mint-now" @click="changeDisplayState(1)">minting</a> one
          </p>
        </div>
              ...
              <div class="card-body">
                ...
                <div class="owner" v-if="token.owner">
                  <p>
                    ...
                    <span v-if="token.owner !== accounts[0].address">{{
                      token.owner
                    }}</span>
    ...
    <div class="minter" v-if="currentState == MINT">
      ...
      <div class="minting-form">
        <div class="name">
          ...
          <input
            v-model="metadata.ipfsMetadata.name"
            name="nft_name"
            class="form-control"
            type="text"
        ...
        <div class="description">
          ...
          <textarea
            v-model="metadata.ipfsMetadata.description"
            name="nft_descr"
            class="form-control"
          ></textarea>
        </div>
        ...
        <div class="image">
          ...
          <div
            class="dropzone"
            :class="{ ok: files.length, waiting: !files.length }"
            @dragover="dragover"
            @dragleave="dragleave"
            @drop="drop"
          >
            ...
            <label for="assetsFieldHandle" class="block cursor-pointer">
              <div v-if="!files.length">
                <p class="instr-t">Drag and drop NFT image at here</p>
              </div>
            </label>
          ...
          <div class="controls minting-controls">
            <button
              ...
              :disabled="
                !files.length ||
                !metadata.ipfsMetadata.description ||
                !metadata.ipfsMetadata.name ||
                isMinting
              "
            >
              Mint NFT
        ...
        <div class="card-deck">
          <div class="card" v-for="(token, i) in myNfts" :key="i">
            ...
            <div class="card-body">
              ...
              <div class="owner" v-if="token.owner">
                <p><strong>Owned by:</strong>&nbsp;<span>You</span></p>
              </div>
              <div class="controls transfer-controls">
                ...
                <!-- Transfer NFT -->
                <div
                  :class="'transfer transfer-' + i"
                  v-if="transferring.tokenId == token.id"
                >
                  <!-- Token: ID -->
                  <label :for="'nft_id' + i"><strong>ID:</strong></label>
                  <input
                    v-model="transferring.tokenId"
                    :name="'nft_id_' + i"
                    class="form-control"
                    type="text"
                    readonly
                  ...
                  <label class="recipient" :for="'nft_transfer_' + i"
                    ><strong>Recipient:</strong></label
                  >
                  <input
                    v-model="transferring.recipient"
    ...
    <div
      class="nfts address"
      v-if="currentState == VIEW_OWNER && selectedOwner"
    >
    ...
    <div class="controls logs" v-if="logs.length && !showLogs">
      <button class="btn btn-inverse" @click="showLogs = true">
        Show Logs
      </button>
    </div>
    ...
    <div class="logs" v-if="logs.length && showLogs">
      <div class="controls">
        <button class="btn btn-inverse" @click="showLogs = false">
          Hide Logs
        </button>
      </div>
      <div v-for="(log, i) in logs" :key="i">
        <p class="label" v-if="log.timestamp">
          <strong>
            ...
            <span v-if="log.transfer">Transferred NFT:&nbsp;</span>({{
              log.timestamp
            }}):
          </strong>
        </p>
  ...
  methods: {
    ...
    ipfsUpload: async function () {
      ...
      reader.onload = async (event) => {
        ...
        try {
          ...
          this.metadata.ipfsMetadata.image =
            IPFS_PREFIX + String(uploadResult.cid);
          +IPFS_SUFFIX;
    ...
    getTokenMeta: async function (tokenId = false) {
      if (!tokenId || typeof tokenId !== "string") {
        console.warn(
          "Invalid token ID. Token ID must be a string, but got " +
            typeof tokenId
        );
        return;
      }
  ...
  computed: {
    myNfts: function () {
      ...
      return this.nfts.market.tokens.filter((token) => {
        if (token.owner) {
          if (token.owner == this.accounts[0].address) return token;
        }
      });
    },
  },
