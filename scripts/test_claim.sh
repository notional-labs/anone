...
ADDRESS=$(anoned keys show $KEY -a)
echo $ADDRESS
...
cat $HOME/.anone/config/genesis.json | jq '.app_state["claims"]["params"]["airdrop_start_time"]="2022-04-13T10:44:41.539339834Z"' > $HOME/.anone/config/tmp_genesis.json && mv $HOME/.anone/config/tmp_genesis.json $HOME/.anone/config/genesis.json
cat $HOME/.anone/config/genesis.json | jq '.app_state["claims"]["module_account_balance"]["amount"]="1000000000000"' > $HOME/.anone/config/tmp_genesis.json && mv $HOME/.anone/config/tmp_genesis.json $HOME/.anone/config/genesis.json
cat $HOME/.anone/config/genesis.json | jq '.app_state["claims"]["claim_records"][0] |= .+ {"address": "'$ADDRESS'", "initial_claimable_amount": [{"denom": "uan1", "amount": "1000000000000"}], "action_completed": [true, false, false, false]}' > $HOME/.anone/config/tmp_genesis.json && mv $HOME/.anone/config/tmp_genesis.json $HOME/.anone/config/genesis.json
